...
import { getDivisions } from '@/api/api' // Adjust the import path as needed
...
// Replace the hardcoded divisions with a ref
...
onMounted(async () => {
  try {
    ...
    divisions.value = response.data // Assign the fetched data to divisions
  } catch (err) {
    error.value = 'Failed to fetch divisions. Please try again later.'
    console.error('Error:', err)
  } finally {
    isLoading.value = false // Set loading to false after the API call completes
  }
})
...
const wrapIndex = (index: number) => {
  const len = divisions.value.length
...
const visibleDivisions = computed(() => {
  return divisions.value.map((_, i) => {
    ...
    return {
      ...
      zIndex: divisions.value.length - Math.abs(i - 2),
      translateY: (i - 2) * 60,
